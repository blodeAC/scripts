      ...
      for _, keyValue in ipairs(allValueStrings) do
        if lootable then
          ...
          for j = #ruleItem.sorted[keyValue], 1, -1 do
            ...
            local itemValue = item[keyValue] and
            (item[keyValue][keyForCompare] or item[keyValue][ExtraEnums[keyValue][keyForCompare]] or nil)
...
local function processItem(item)
  local weenie = game.World.Get(item.id)
  for l = #inspectQueue, 1, -1 do
    if inspectQueue[l] == item.id then
      local winningLootRule = evaluateLoot(item)
      if winningLootRule then
        loot(item, winningLootRule, weenie)
      end
...
game.World.OnContainerOpened.Add(function(containerMsg)
  if containerMsg.Container.ObjectClass == ObjectClass.Corpse or containerMsg.Container.Name == "Corpse" or Regex.IsMatch(containerMsg.Container.Name, "Chest") then
    local containerId = containerMsg.Container.Id
   
    local trimmedList
    if (#game.World.OpenContainer.AllItemIds == 0 and container[containerId] ~= nil) then
      trimmedList = {}
      for i, itemId in ipairs(container[containerId]) do
        if game.World.Exists(itemId) and game.World.Get(itemId).Container ~= game.Character.Weenie then
          table.insert(trimmedList, itemId)
        end
      end
    end

    if container[containerId] == nil then
      container[containerId] = containerMsg.Container.AllItemIds
      game.World.Get(containerId).OnDestroyed.Add(function()
        container[containerId] = nil
      end)
    else
      container[containerId] = trimmedList
    end
...
function prettyPrintJSON(value, indent)
        ...
        for i = #item.sorted[valuesKey], 1, -1 do
          if currentRow >= visibleStartRow and currentRow <= visibleEndRow then
            ...
            if value ~= nil then
              ...
              ImGui.Text(tonumber(keyForText) and ExtraEnums[valuesKey][keyForText] or keyForText)
              ImGui.TableNextColumn()
...
hud.OnRender.Add(function()
  ...
  if inspectedItem == nil then
    ...
    if ImGui.BeginTabBar("InspectedItemTabs") then
      ...
      Vector2.new(ImGui.GetContentRegionMax().X - checkBoxSize.X * 2 - ImGui.CalcTextSize(testModeLabelText).X, 0))
      ...
      if ImGui.BeginTabItem(inspectedItem.name) then
        ...
        ImGui.SetNextItemWidth(ImGui.GetContentRegionMax().X - (ImGui.GetCursorScreenPos().X - ImGui.GetWindowPos().X) *
        2.5)
        local changed, newCharIndex = ImGui.Combo("##importServerCharacter", importIndex, availableToImport,
          #availableToImport)
        if changed then
    ...
    for i, rule in ipairs(lootRules) do
      ...
      if rule.isRenaming then
        ...
        elseif rule.isShown then
          ...
          local changed, newValue = ImGui.InputText("##keep" .. i, tostring(rule.keepCount or -1), 3,
            _imgui.ImGuiInputTextFlags.CharsDecimal + _imgui.ImGuiInputTextFlags.EnterReturnsTrue)
          if changed then
      ...
      if not rule.isShown then
        ...
        Vector2.new(ImGui.GetColumnWidth() - ImGui.CalcTextSize(tostring(rule.totalFound)).X, 0))
      ...
      if rule.isShown then
        ...
        if changed then
          ...
          for idx, itemList in ipairs(sortedEnums) do
            ...
            rule[enumId] = rule[enumId] or
            1                                -- Default to 1 if not set, or another fallback value, this is the sort index
...
loadWindowData()
hud.Visible = (windowData.hud == nil and true) or (windowData.hud.isVisible ~= false and true) or false
lootRuleHolder.Visible = (windowData.lootRuleHolder == nil and true) or
(windowData.lootRuleHolder.isVisible ~= false and true) or false
